...
4.Create two synchronized methods:
  ->printHello() → prints “Hello” when it's its turn; otherwise waits.
  ->printWorld() → prints “World” when it's its turn; otherwise waits.
5.In each method:
  ->Use while() loop to check whose turn it is.
  ->Use wait() to block if not its turn.
